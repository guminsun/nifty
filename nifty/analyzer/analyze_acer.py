...
def analyze_acer_card_2(card_2, module):
    ...
    analyze_acer_card_2_iopt(env.next(stmt_iter), card_2, module)
    ...
    analyze_acer_card_2_nxtra(env.next(stmt_iter), card_2, module)
    ...
    if nxtra_r_value > 0:
        ...
        if env.is_even(stmt_len):
            ...
            for i in range(0, stmt_len, 2):
                analyze_acer_card_4_iz(index, env.next(stmt_iter), card_4,
                                       module)
                analyze_acer_card_4_aw(index, env.next(stmt_iter), card_4,
                                       module)
                index += 1
        else:
            msg = ('expected an even number of statements in \'card_4\' ' +
                   'since \'nxtra\' (previously defined in \'card_4\') ' +
...
def analyze_acer_card_5(card_2, card_5, module):
    ...
    iopt_node = env.get_identifier('iopt', card_2)
    iopt_r_value = env.get_value(env.get_r_value(iopt_node))
    if iopt_r_value == 1:
        ...
        analyze_identifier_matd(env.next(stmt_iter), card_5, module)
        analyze_identifier_tempd(env.next(stmt_iter), card_5, module)
        rule.no_more_statement_allowed(env.next(stmt_iter), card_5, module)
    else:
...
def analyze_identifier_matd(matd_node, card, module):
    rule.identifier_must_be_defined(('matd', None), matd_node, card, module)
    return matd_node

def analyze_identifier_tempd(tempd_node, card, module):
    # XXX: Temperature does not have to be defined. Defaults to 300.
    if env.not_defined(tempd_node):
        return tempd_node
    else:
        # If 'tempd_node' is defined, make sure it's a tempd node.
        rule.identifier_must_be_defined(('tempd', None), tempd_node, card,
                                        module)
    return tempd_node
...
def analyze_acer_card_6(card_2, card_6, module):
    ...
    if iopt_r_value == 1:
        ...
        rule.no_more_statement_allowed(env.next(stmt_iter), card_6, module)
    else:
...
def analyze_acer_card_7(card_2, card_7, module):
    ...
    if iopt_r_value == 1:
        ...
        rule.no_more_statement_allowed(env.next(stmt_iter), card_7, module)
    else:
...
def analyze_acer_card_8(card_2, card_8, module):
    ...
    if iopt_r_value == 2:
        ...
        analyze_identifier_matd(env.next(stmt_iter), card_8, module)
        analyze_identifier_tempd(env.next(stmt_iter), card_8, module)
        analyze_acer_card_8_tname(env.next(stmt_iter), card_8, module)
        rule.no_more_statement_allowed(env.next(stmt_iter), card_8, module)
    else:
...
def analyze_acer_card_8a(card_2, card_8a, module):
    ...
    if iopt_r_value == 2:
        ...
        rule.no_more_statement_allowed(env.next(stmt_iter), card_8a, module)
    else:
        # Prepare a descriptive message of why card_8a should not defined.
        msg = 'since iopt = ' + str(iopt_r_value) + ' in \'card_2\''
        rule.card_must_not_be_defined('card_8a', module, msg)
    return card_8a
...
def analyze_acer_card_9(card_2, card_9, module):
    ...
    if iopt_r_value == 2:
        ...
        rule.no_more_statement_allowed(env.next(stmt_iter), card_9, module)
    else:
...
def analyze_acer_card_10(card_2, card_10, module):
    ...
    if iopt_r_value == 3:
        ...
        analyze_identifier_matd(env.next(stmt_iter), card_10, module)
        analyze_identifier_tempd(env.next(stmt_iter), card_10, module)
        rule.no_more_statement_allowed(env.next(stmt_iter), card_10, module)
    else:
...
def analyze_acer_card_11(card_2, card_11, module):
    ...
    if iopt_r_value in range(4,6):
        ...
        analyze_identifier_matd(env.next(stmt_iter), card_11, module)
        rule.no_more_statement_allowed(env.next(stmt_iter), card_11, module)
    else:
